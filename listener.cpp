...
#ifndef WIN32
SocketListener::SocketListener(NetworkInterface *iface, std::string unixSocket, void* zmq): interfarce(iface) {
	...
	int rc = zmq_bind(zmq_broadcast, (std::string("ipc://") + unixSocket + "broadcast").c_str()); assert (rc == 0);
}
#endif
...
void SocketListener::checkFds(fd_set *readset, Tox *my_tox, ToxVPNCore *toxvpn) {
...
void SocketListener::broadcast(const char* msg) {
	...
	strcpy(hack, "all");
	hack[3] = 0;
	int rc = zmq_msg_init_data(&header, hack, 3, NULL, NULL); assert(rc == 0);
	...
	zmq_msg_t msg_out;
	rc = zmq_msg_init_data(&msg_out, (void*)copy, strlen(msg), NULL, NULL); assert(rc == 0);
